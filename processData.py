...
def downloadFile(date, filesource, dataquery,filenamePrefix, folder):
    """Download data from CDC website"""

    client = Socrata("data.cdc.gov", None)
    results = client.get(filesource, query=dataquery)
    results_df = pd.DataFrame.from_records(results)
    filename = './{}/{}-{}.csv'.format(folder,filenamePrefix,date)
    results_df.to_csv(filename)

def processPopData(input_file):
    ...
    merge.loc[merge['State']=='Total U.S. (including D.C. and territories)','State or Region Code']='US'
    output = merge[(merge['State or Region Code'].isnull()==False)&(merge['Population'].isnull()==False)][['State or Region Code','Population']]
...
def check_download(date, filesource, dataquery, filenamePrefix, folder):
    ...
    datelist = [datetime.datetime.strptime(file.split('.')[0][-10:], '%Y-%m-%d')  for file in os.listdir(folder)]
    ...
    for dt in missing_dates:
        ...
        downloadFile(dt.date(), filesource, query, filenamePrefix, folder)
...
def consolidate_state_data():
    ...
    path = './StateData'
    usDataDf = pd.DataFrame(columns=['Date','state','tot_cases', 'new_case','tot_death','new_death'])
    for file in os.listdir('./StateData'):
        ...
        df[['tot_cases', 'new_case','tot_death','new_death']] = df[['tot_cases', 'new_case','tot_death','new_death']].apply(pd.to_numeric)
        df.loc['US'] = df.sum(numeric_only=True)
        df.loc['US','state']='US'
        df.loc['US','Date'] = dt
        df.loc['US','submission_date']=dt
        usDataDf=usDataDf.append(df[['Date','submission_date','state','tot_cases', 'new_case','tot_death','new_death']])
    ...
    pop = processPopData('./data/pop.html')
    newrow = {'State or Region Code':'NYC', 'Population':8336817}
    pop=pop.append(newrow, ignore_index=True)
    test = pd.merge(left=usDataDf, right=pop, left_on='state',right_on='State or Region Code', how='outer')
    ...
    test['7 day average new cases with 7 day lag']=100000*(test['new_case'].shift(7).rolling(window=7).mean())/test['Population']
